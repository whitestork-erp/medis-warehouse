...
frappe.ui.form.on("Delivery Route Item", {
    invoice_number: function(frm, cdt, cdn) {
        ...
        frappe.db.get_doc("Sales Invoice", child.invoice_number)
            .then(invoice => {
                ...
                frappe.db.get_value("Invoice Status Updater", 
    ...
    number_packed: function(frm, cdt, cdn) { update_summary(frm); }
});

frappe.ui.form.on('Delivery Route Item', {
    // When invoice_number is filled
    invoice_number: function(frm, cdt, cdn) {
        ...
        if (row === last_row && row.invoice_number) {
            ...
            setTimeout(() => {
                let grid = frm.fields_dict['delivery_route_items'].grid;
                    ...
                    let input = grid.wrapper.find(`.grid-row[data-idx="${row.idx}"] [data-fieldname="invoice_number"] input`);
    ...
    refresh: function(frm) {
        ...
        // Attach a focus event listener to the first row’s input
        setTimeout(() => {
            let grid = frm.fields_dict['delivery_route_items'].grid;
            if (grid.grid_rows.length > 0) {
                let first_input = grid.wrapper.find(`.grid-row[data-idx="1"] [data-fieldname="invoice_number"] input`);
                if (first_input && first_input.length) {
                    first_input.off("focus.add_row").on("focus.add_row", function () {
                        // If only one row exists, add another when first row is focused
...
frappe.ui.form.on("Delivery Route", {
    refresh: function(frm) {
        ...
        frm.fields_dict["delivery_route_items"].grid.get_field("invoice_number").get_query = function(doc, cdt, cdn) {
            // collect already selected invoices in child table
            let selected_invoices = (doc.delivery_route_items || []).map(row => row.invoice_number);
        ...
        if (frm.doc.workflow_state === "Out For Delivery" && !frm.doc.driver) {
            frappe.msgprint({
                ...
                indicator: "red",
                message: __("You must assign a driver before setting this Delivery Route to <b>Out For Delivery</b>.")
            });
        ...
        frm.doc.delivery_route_items = frm.doc.delivery_route_items.filter(row => row.invoice_number);
        frm.refresh_field('delivery_route_items');
     ...
     delivery_route_items_add: function(frm) { update_summary(frm); },
    delivery_route_items_remove: function(frm) { update_summary(frm); },
    delivery_route_items_on_form_rendered: function(frm) { update_summary(frm); },
                ...
                transitions.forEach((transition) => {
                    ...
                    if (hasAccess) {
                        listview.page.add_inner_button(transition.action, () => {
                            ...
                            const new_route = `/app/${route}/new-${frappe.model.scrub("Delivery Route Status Updater")}-${frappe.utils.get_random(10)}?target_state=${target_state}`;
